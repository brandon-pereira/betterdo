...
export async function createList(listObj: Partial<List>, { db, user }: RouterOptions): Promise<ListDocument> {
...
export async function updateList(
  ...
  if (
    ...
    (!Array.isArray(updatedList.tasks) ||
      ...
      updatedList.tasks.find(_id => !list.tasks.map(task => task._id.toString()).includes(_id)))
  ) {
  ...
  if (updatedList.members && Array.isArray(updatedList.members)) {
    ...
    const newMembers = updatedList.members.filter((member: string) => !currentMembers.includes(member));
...
export async function deleteList(listId: ObjectId | string, { db, user }: RouterOptions): Promise<GenericStatus> {
  // Ensure list id is passed
  if (!listId) throwError("Invalid List ID");
  // Convert ListId to object if not
  if (typeof listId === "string") {
