...
export async function updateTask(
  ...
  if (updatedTask.isCompleted !== undefined && task.isCompleted !== updatedTask.isCompleted) {
    if (updatedTask.isCompleted) {
      ...
      notifyAboutSharedList(`${user.firstName} completed ${task.title} in ${list.title}.`, list, {
        notifier,
        user,
        db
      });
...
export async function getTask(taskId: ObjectId | string, { db, user }: RouterOptions): Promise<TaskDocument> {
  // Ensure list id is passed
  if (!taskId) throwError("Invalid Task ID");
  // Get task
