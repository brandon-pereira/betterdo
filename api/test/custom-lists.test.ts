...
describe("Custom Lists API", () => {
  ...
  test("Inbox shouldn't allow deletion", async () => {
    await expect(deleteList(inbox._id, router1)).rejects.toThrowErrorMatchingInlineSnapshot(`"Invalid List ID"`);
  });
  ...
  test("High priority list should only return valid tasks", async () => {
    ...
    await createTask(validList2._id, { title: "Invalid because wrong user", priority: "high" }, router2);
  ...
  test("Should allow returning additional tasks", async () => {
    const task = await createTask("highPriority", { title: "title", isCompleted: true }, router1);
