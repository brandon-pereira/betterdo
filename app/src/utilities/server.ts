...
export const createList = (list: { title: string; color: string }): Promise<List> => {
  return _put<List>(`lists`, list);
};

export const updateTask = (taskId: string, updatedProps: Partial<Task>): Promise<Task> => {
  return _post<Task>(`tasks/${taskId}`, updatedProps);
};

export const updateList = (listId: string, updatedProps: ServerList): Promise<List> => {
 ...
 */
function _get<response>(url: string, queryParameters: { [key: string]: string } = {}): Promise<response> {
...
function _post<response, args = object>(url: string, data: args): Promise<response> {
  return _fetch(`${baseUrl}/${url}`, {
    method: "POST",
...
function _put<response, args = object>(url: string, data: args): Promise<response> {
  return _fetch(`${baseUrl}/${url}`, {
    method: "PUT",
    body: JSON.stringify(data)
  });
