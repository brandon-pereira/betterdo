...
import { Icon, Container, Color, LabelContainer, TooltipContainer } from "./ColorPicker.styles";
...
function ColorPicker({ value, onChange }: Props) {
  ...
  const onRefreshPalette = useCallback(
    (e?: React.MouseEvent<HTMLDivElement>) => {
      ...
      setIndex(0);
      setPalette(palette.map((curr, i) => (i === 0 ? currentColor : randomColor())));
    },
    [palette, index]
  );

  const onLaunchPicker = useCallback((e?: React.MouseEvent<HTMLDivElement>) => {
    if (e) {
      e.preventDefault();
    }
  ...
  useEffect(() => {
    if (colorPaletteRef.current) {
      const events = new TouchEvents(colorPaletteRef.current, ({ direction }) => {
  ...
  return (
    <>
      <LabelContainer>
        ...
        <Icon size="1rem" icon={Refresh} color="currentColor" onClick={onRefreshPalette}>
          Refresh Palette
        </Icon>
        <TooltipContainer>
          <Icon size="1rem" icon={Eyedropper} color="currentColor" onClick={onLaunchPicker}>
            Pick a colour
          </Icon>
          <input
            ref={inputColorRef}
