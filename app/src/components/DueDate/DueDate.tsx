...
import { nextMonday, isSameDay, startOfToday, startOfTomorrow, isSameWeek } from "date-fns";

import { Container, DayIcon, ItemContainer, ItemLabel, DueDateInput } from "./DueDate.styles";
      ...
      <Container>
        ...
        <ItemContainer selected={Boolean(isOther)} onClick={_onCustomDateSelect}>
          <DueDateInput
            ...
            onChange={e => onChange(formatDateValueToDate(e.target.value))}
          />
          <Icon size={`30px`} color="currentColor" icon={Eyedropper} />
...
const formatDateValueToDate = (value: string) => {
  if (
    ...
    return new Date(parseInt(year), parseInt(month) - 1, parseInt(day)).toUTCString();
  }
  return "";
};
