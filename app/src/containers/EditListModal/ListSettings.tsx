...
function ListSettings({ setUnsavedChanges, onRequestClose }: Props) {
  ...
  const deleteList = useCallback(async () => {
    const result = confirm(`Are you sure you want to delete the list "${state.title}"? This can't be undone.`);
    if (result) {
      setDeleting(true);
      setInvalid(false);
      try {
  ...
  const onSubmit = useCallback(
    ...
    [onRequestClose, setUnsavedChanges, isSaving, currentListId, state, modifyList, isDeleting]
  );
